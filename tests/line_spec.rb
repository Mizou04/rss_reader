...
RSpec.describe RSS::Line do
	...
	it "renders line" do
	line = RSS::Line.new(@smallContent)
	expect(line.render().join("\n")).to eq("#{" " * (RSS::DefPad)}#{@smallContent}")
	end
	it "given padding=true it prints string with left padding #{RSS::Padding + RSS::DefPad}" do
	line = RSS::Line.new(@smallContent, true)
	expect(line.render().join("\n")).to eq("#{" " * (RSS::Padding + RSS::DefPad)}#{@smallContent}")
	end
	...
	it "given smaller content than window size (#{$WindowSize}) with no padding, split to chunks" do
		...
		expect(line.render()[0]).to eq("#{" " * RSS::DefPad}#{@smallContent}")
	end
